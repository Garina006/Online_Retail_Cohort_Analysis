...
SELECT 
  ...
  , ROUND(SUM(ROUND(avg_check * num_customers, 2)) OVER (PARTITION BY Cohort ORDER BY first_day_of_month) / 
      FIRST_VALUE(num_customers) OVER (PARTITION BY Cohort ORDER BY first_day_of_month), 2) AS arpu
FROM
  MonthlyData
ORDER BY
...
FROM
  Resume
PIVOT (
  SUM(arpu) FOR formatted_first_day_of_month IN (
