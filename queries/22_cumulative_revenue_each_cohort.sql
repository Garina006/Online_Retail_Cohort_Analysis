...
SELECT 
  ...
  , SUM(ROUND(avg_check * num_customers, 2)) OVER (PARTITION BY Cohort ORDER BY first_day_of_month) AS cumulative_income
FROM
  MonthlyData
ORDER BY
...
FROM
  Resume
PIVOT (
  SUM(cumulative_income) FOR formatted_first_day_of_month IN (
